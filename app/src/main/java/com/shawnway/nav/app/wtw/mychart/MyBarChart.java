...
public class MyBarChart extends BarChart {
    ...
    public void setInternationalMarker(MyLeftMarkerView leftMarkerView, MyRightMarkerView rightMarkerView, MyBottomMarkerView bottomMarkerView, InternationalDataParse mData) {
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            if (xIndex <= deltaX && xIndex <= deltaX * mAnimator.getPhaseX()) {
                ...
                String data1 = "";
                String data2 = "";
                if (minuteHelper != null) {
                    data1 = minuteHelper.getDatas().get(mIndicesToHighlight[i].getXIndex()).time;
                    myMarkerViewLeft.setNumType(MyLeftMarkerView.TYPE_WP);
                }
                if (minuteHelper2!=null&&minuteHelper2.getDatas().size() > 0) {
                    leftData = minuteHelper2.getDatas().get(mIndicesToHighlight[i].getXIndex()).cjnum;
                    data2 = minuteHelper2.getDatas().get(mIndicesToHighlight[i].getXIndex()).time;
                    myMarkerViewLeft.setNumType(MyLeftMarkerView.TYPE_INTERNATIONAL_BAR);
                }
                if (minuteHelper2!=null&&minuteHelper2.getKLineDatas().size() > 0) {
                    leftData = minuteHelper2.getKLineDatas().get(mIndicesToHighlight[i].getXIndex()).vol;
                    data2 = minuteHelper2.getKLineDatas().get(mIndicesToHighlight[i].getXIndex()).date;
                    myMarkerViewLeft.setNumType(MyLeftMarkerView.TYPE_INTERNATIONAL_BAR);
                }
                myMarkerViewLeft.setData(leftData);
                mMyBottomMarkerView.setData(String.format("%s", data1.endsWith("") ? data2 : data1));
